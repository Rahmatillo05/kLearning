    ...
    public function actionIndex()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                $model->first_image = $first_image ? UploadFile::saveFile($first_image) : $model->getOldAttribute('first_image');
                $last_image = UploadedFile::getInstance($model, 'last_image');
                $model->last_image = $last_image ? UploadFile::saveFile($last_image) : $model->getOldAttribute('last_image');
                if ($model->save()) {
